...
- **contract_review**: 审核合同文件，生成审核报告（自包含黑盒：内部完成解析→审核→报告→保存）。可传 file_path 或 doc_id，返回 report_id、report_path、summary 和 risk_count。
- **document_retrieval**: 在已入库的合同文档中检索具体条款内容。必须带 doc_id 范围过滤。
- **list_documents**: 列出已入库的合同文档及状态。
- **batch_ingest_documents**: 批量上传多个文件入库（异步后台处理）。传入路径列表，返回 batch_id 和每文件状态。
- **batch_ingest_status**: 查询批量入库进度。传入 batch_id，返回每个文件的处理状态。
...
你只需要做：确认文件路径或已入库 doc_id → 调 contract_review(file_path 或 doc_id, contract_name) → 拿到结果后汇报。
**你自己不参与审核分析**，contract_review 是独立黑盒。

如果用户从 list_documents 的列表中确认“第一个/第二个/某个文档ID”，应优先使用该文档的 doc_id 调用 contract_review，不要把文件名当作 file_path。
用户提到的“合同编号/协议编号/订单号/发票号”通常是合同正文中的业务字段，不是系统 doc_id。不要拿它直接和 DOC-xxxx 文档ID或文件名比较；应先用 document_retrieval 搜索该编号，命中后再根据返回的 doc_id 绑定文档。
...
你需要做：
- 先判断是否能唯一确定 doc_id。
- 能唯一确定时，再调用 document_retrieval(query=..., doc_id=...)。
- 不能唯一确定时，先调用 list_documents，并向用户展示最近 3 份候选文档进行确认。

### 3. 文件列表 → 调 list_documents
...
def build_contract_sub_agent(base_dir: Path) -> Any:
    ...
    子 Agent 拥有 contract_review、document_retrieval、list_documents、batch_ingest_documents、batch_ingest_status 工具，
    在独立上下文中处理合同相关任务。
    """
    from config import get_settings
    from tools.contract_review_tool import ContractReviewTool
    from tools.document_batch_tool import DocumentBatchIngestTool, DocumentBatchStatusTool
    ...
    sub_tools: list[BaseTool] = [
        ...
        DocumentBatchIngestTool(root_dir=base_dir),
        DocumentBatchStatusTool(root_dir=base_dir),
    ]
