...
`memory_module_v2` 的 PostgreSQL + pgvector 存储设计，包含长期记忆的 exchange 分段、LLM 蒸馏对象和向量索引结构。

![Database Schema](database_schema.png)

### Langfuse 可观测性追踪

通过 Langfuse 可视化 Agent 的每轮推理链路，包括 Guardian 安全判定、LLM 调用、工具执行与 token 用量监控。

![Guardian Test & Langfuse Trace](guardain_test_langfuse.png)
...
### 内置 Agent 工具
- `terminal`：沙箱化 shell 执行（危险命令黑名单 + 超时控制）
- `python_repl`：Python 代码片段子进程执行
- `fetch_url`：HTTP 请求，HTML → Markdown 转换
- `read_file`：项目根目录下安全文件读取（含路径穿越检测）
- `search_knowledge_base`：基于 llama-index 的本地知识库语义检索
- `search_memory` / `distill_session`（v2 专属）：跨会话记忆检索与手动蒸馏
